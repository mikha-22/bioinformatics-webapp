...
import { Textarea } from "@/components/ui/textarea"; // <<< ADDED for Run Description
import {
  Select,
  SelectContent,
  SelectItem,
...
// <<< NEW: Common schema part for run_name and run_description >>>
const commonRunInfoSchema = {
  run_name: z.string()
    .min(1, "Run Name is required.")
    .max(100, "Run Name must be 1-100 characters.")
    .regex(/^[a-zA-Z0-9_ -]+$/, "Run Name can contain letters, numbers, spaces, underscores, and hyphens."), // Allow spaces initially
  run_description: z.string().max(250, "Run Description must be at most 250 characters.").optional(),
};
// <<< END NEW >>>
...
const bamCramSampleSchema = z.object({
  ...
  .refine(data => {
    if (!data.index || !data.bam_cram) return true;
    if (data.bam_cram.endsWith('.cram') && !data.index.endsWith('.crai')) return false;
    if (data.bam_cram.endsWith('.bam') && !data.index.endsWith('.bai')) return false;
    return true;
...
const vcfSampleSchema = z.object({
  ...
  .refine(data => {
    ...
    return data.index.endsWith('.tbi') || data.index.endsWith('.csi');
...
const STEPS_FOR_INPUT_TYPE: Record<'fastq' | 'bam_cram' | 'vcf', SarekStep[]> = {
  fastq: ["mapping"],
...
const fastqPipelineSchemaBase = z.object({
  ...
  step: z.enum(['mapping'], { required_error: "Starting step is required for FASTQ input." }),
  ...
  description: z.string().optional(), // Sarek's internal description
});
...
const vcfPipelineSchemaBase = z.object({
  ...
  genome: z.enum(VALID_GENOME_VALUES, { required_error: "Genome build is required" }),
  step: z.enum(['annotation'], { required_error: "Starting step is required for VCF input." }),
  intervals_file: z.string().optional().refine((val) => !val || val === "" || val.endsWith('.bed') || val.endsWith('.list') || val.endsWith('.interval_list'), { message: "Intervals file must end with .bed, .list, or .interval_list" }),
  dbsnp: z.string().optional(),
  known_indels: z.string().optional(),
  pon: z.string().optional(),
...
const ADVANCED_FIELD_NAMES: (keyof PipelineFormValues)[] = [
  'tools', 'profile', 'aligner', 'pon', 'trim_fastq', 'joint_germline', 'skip_qc', 'skip_annotation'
];
...
export default function InputPage() {
  ...
  const form = useForm<PipelineFormValues>({
    ...
    defaultValues: {
      run_name: "", // <<< ADDED default
      run_description: "", // <<< ADDED default
      input_type: 'fastq',
      ...
      description: "", // Sarek's internal description
    },
  });
  ...
  useEffect(() => {
    if (watchedInputType !== selectedInputType) {
      ...
      const newDefaultValues: Partial<PipelineFormValues> = {
        run_name: "", // Reset run_name on input type change
        run_description: "", // Reset run_description on input type change
        ...
        description: currentValues.description || "", // Sarek's internal description
  ...
  const stageMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      form.reset(); // This will reset run_name and run_description
    ...
    onError: (error) => {
      ...
      const detail = (error as any).originalError?.response?.data?.detail;
  ...
  function onSubmit(values: PipelineFormValues) {
    const apiPayload: PipelineInput = {
      run_name: values.run_name, // <<< ADDED
      run_description: values.run_description, // <<< ADDED
      ...
      intervals_file: values.intervals_file || undefined, dbsnp: values.dbsnp || undefined,
      known_indels: values.known_indels || undefined, pon: values.pon || undefined,
      ...
      description: values.description || undefined, // Sarek's internal description
    };
      ...
      if (firstErrorKey === 'samples') {
        ...
        if (Array.isArray(errors.samples)) {
          ...
          if (firstSampleErrorIndex !== -1) {
            ...
            if (sampleErrors) { const firstSampleFieldError = Object.keys(sampleErrors)[0] as keyof ApiSampleInfo; fieldNameToQuery = `samples.${firstSampleErrorIndex}.${firstSampleFieldError}`; }
          }
  ...
  const onFormError: SubmitErrorHandler<PipelineFormValues> = (errorsArgument) => {
    console.warn("Form validation failed.", errorsArgument);
    toast.error("Please fix the validation errors.", { duration: 5000 });
  ...
  const toggleCheckboxValue = (fieldName: keyof PipelineFormValues | 'tools', tool?: string) => {
    if (fieldName === 'tools' && tool) {
      ...
      const newVal = currentVal.includes(tool) ? currentVal.filter((t) => t !== tool) : [...currentVal, tool];
      form.setValue("tools", newVal, { shouldValidate: true, shouldDirty: true });
    } else if (fieldName !== 'tools') {
      const fieldKey = fieldName as keyof PipelineFormValues;
      if (fieldKey in form.getValues()) {
        const currentVal = form.getValues(fieldKey);
  ...
  const handleProfileLoaded = (name: string | null, data: ProfileData | null) => {
    ...
    // Reset run_name and run_description when a profile is loaded or defaults are set
    form.setValue('run_name', '', { shouldValidate: false, shouldDirty: true }); // Mark as dirty
    form.setValue('run_description', '', { shouldValidate: false, shouldDirty: true }); // Mark as dirty

    if (data) {
      let loadedInputType: InputType = 'fastq'; // Default to fastq if step is unusual
      ...
      if (loadedInputType !== currentFormInputType) {
        ...
        Object.entries(data).forEach(([key, value]) => {
          ...
          if (fieldKey in form.getValues()) {
            form.setValue(fieldKey as any, value !== null ? value : (currentFormDefaults as any)[fieldKey], { shouldValidate: true, shouldDirty: true });
            if (ADVANCED_FIELD_NAMES.includes(fieldKey as keyof PipelineFormValues) && value !== null) {
              const defaultValueForField = (currentFormDefaults as any)[fieldKey];
      ...
      form.reset(); // This will also reset run_name and run_description due to defaultValues
      setSelectedInputType('fastq');
      setAdvancedAccordionValue(undefined);
    }
  };
  ...
  useEffect(() => {
    if ((formRef.current as any)?._profileToApplyAfterReset) {
      ...
      Object.entries(dataToApply).forEach(([key, value]) => {
        const fieldKey = key as keyof ProfileData;
        if (fieldKey in form.getValues()) {
          form.setValue(fieldKey as any, value !== null ? value : (currentFormDefaultsAfterReset as any)[fieldKey], { shouldValidate: true, shouldDirty: true });
          if (ADVANCED_FIELD_NAMES.includes(fieldKey as keyof PipelineFormValues) && value !== null) {
            const defaultValueForField = (currentFormDefaultsAfterReset as any)[fieldKey];
  ...
  }, [form.formState.isSubmitSuccessful, form, advancedAccordionValue]); // Added advancedAccordionValue to deps

  const handleSaveProfile = async (profileName: string) => {
    const currentValues = form.getValues();
    const profileData: ProfileData = {
      genome: currentValues.genome, step: currentValues.step,
      intervals_file: currentValues.intervals_file || null, dbsnp: currentValues.dbsnp || null,
      known_indels: currentValues.known_indels || null, pon: currentValues.pon || null,
      ...
      description: currentValues.description || null, // Sarek's internal description
    };
    ...
    <FormProvider {...form}>
      <Form {...form}>
        <form ref={formRef} onSubmit={form.handleSubmit(onSubmit, onFormError)} className="space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-xl">Input & Configuration</CardTitle>
              <CardDescription>Select the type of input data and load any saved configurations.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <ProfileLoader
                ...
                currentInputType={selectedInputType}
              />
            </CardContent>
          </Card>

          {/* New "Run Information" Card */}
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Run Name <span className="text-destructive">*</span></FormLabel>
                    <FormControl><Input placeholder="e.g., Somatic_Analysis_Patient123_Exp1" {...field} /></FormControl>
                    <FormDescription>A unique identifier for this pipeline run. Spaces will be converted to underscores by the backend.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Run Description <span className="text-muted-foreground text-xs">(Optional)</span></FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder="e.g., Somatic variant calling for patient X and Y with custom parameters for tool Z, focusing on high-depth regions."
                        {...field}
          ...
          <Card>
            <CardHeader id="samples-card-header">
            ...
            <CardContent className="space-y-4">
              {fields.map((field, index) => {
                if (selectedInputType === 'fastq') {
                  return <SampleInputGroup key={field.id} index={index} remove={remove} control={form.control as Control<any>} />;
                } else if (selectedInputType === 'bam_cram') {
                  return <BamCramSampleInputGroup key={field.id} index={index} remove={remove} control={form.control as Control<any>} />;
                } else if (selectedInputType === 'vcf') {
                  return <VcfSampleInputGroup key={field.id} index={index} remove={remove} control={form.control as Control<any>} />;
                }
                return null;
              })}
              <Button type="button" variant="outline" size="sm" onClick={addSample} className="mt-2 cursor-pointer" >
                <PlusCircle className="mr-2 h-4 w-4" /> Add Sample
              </Button>
              <FormMessage>{form.formState.errors.samples?.root?.message}</FormMessage>
              {Array.isArray(form.formState.errors.samples) && form.formState.errors.samples.map((sampleError, index) => (
                sampleError && Object.values(sampleError).map((err: any, i) => (
                  err?.message && <FormMessage key={`${index}-${i}`} className="text-xs pl-2">{`Sample ${index + 1}: ${err.message}`}</FormMessage>
                ))
              ))}
            </CardContent>
          </Card>

          {/* Core Pipeline Setup Card */}
          <Card>
            <CardHeader>
              <CardTitle className="text-xl">Core Pipeline Setup</CardTitle>
              <CardDescription>Essential parameters for the Sarek pipeline run.</CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
              <FormField
                control={form.control}
                name="description" // This is Sarek's internal description
                render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    <FormLabel>Sarek Config Description <span className="text-muted-foreground text-xs">(Optional)</span></FormLabel>
                    <FormControl>
                      <Input placeholder="e.g., Custom settings for tumor-normal analysis" {...field} value={field.value ?? ''}/>
                    </FormControl>
                    <FormDescription>An optional description stored within Sarek's configuration/metadata.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                      <FormControl>
                        <SelectTrigger><SelectValue placeholder="Select genome build" /></SelectTrigger>
                      </FormControl>
                      <SelectContent>{SAREK_GENOMES.map(g => (<SelectItem key={g.value} value={g.value}>{g.label}</SelectItem>))}</SelectContent>
                    </Select>
                    <FormDescription className="italic">Select the genome assembly key.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem id="step-form-item">
                    ...
                    <Select onValueChange={field.onChange} value={field.value} disabled={availableSteps.length <= 1} >
                      <FormControl>
                        <SelectTrigger id="step"><SelectValue placeholder="Select starting step" /></SelectTrigger>
                      </FormControl>
                      <SelectContent>{availableSteps.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                    </Select>
              ...
              <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-md border p-4 md:col-span-2 hover:bg-accent/50 transition-colors select-none">
                <FormLabel htmlFor="flag-wes" className="flex flex-row items-center space-x-3 space-y-0 font-normal cursor-pointer w-full h-full">
                  ...
                  <div className="space-y-1 leading-none">
                    <span>Whole Exome Sequencing (WES)</span>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    ...
                    <FormControl>
                      <FileSelector fileTypeLabel="Intervals" fileType="intervals" extensions={[".bed", ".list", ".interval_list"]} value={field.value || undefined} onChange={field.onChange} placeholder="Select intervals file..." allowNone required={false} />
                    </FormControl>
                    <FormDescription className="italic">Target regions.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-md border p-4 md:col-span-2 hover:bg-accent/50 transition-colors select-none">
                  <FormLabel htmlFor="flag-skip_baserecalibrator" className="flex flex-row items-center space-x-3 space-y-0 font-normal cursor-pointer w-full h-full">
                    ...
                    <div className="space-y-1 leading-none">
                      ...
                      <FormDescription className="italic mt-1">(If unchecked, dbSNP or Known Indels file is required below).</FormDescription>
                    </div>
                  </FormLabel>
              ...
              {showSkipBaserecalibrator && !form.watch('skip_baserecalibrator') && (
                <>
                  <FormField
                    control={form.control} name="dbsnp"
                    render={({ field }) => (
                      <FormItem>
                  ...
                  />
                  <FormField
                    control={form.control} name="known_indels"
                    render={({ field }) => (
                      <FormItem>
          ...
          {/* Advanced Sarek Parameters Card */}
              ...
              <Tooltip>
                {isStagingDisabled ? (
                  <TooltipTrigger asChild>
                    <Button type="submit" disabled={true} className={cn("border border-primary hover:underline bg-primary text-primary-foreground opacity-50 cursor-not-allowed")} aria-disabled={true}>
                      {(stageMutation.isPending || saveProfileMutation.isPending) ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
                      Stage Pipeline Run
                    </Button>
                  </TooltipTrigger>
                ) : (
                  <TooltipTrigger asChild>
                    <Button type="submit" disabled={false} className={cn("border border-primary hover:underline bg-primary text-primary-foreground hover:bg-primary/90")}>
                      {(stageMutation.isPending || saveProfileMutation.isPending) ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
                      Stage Pipeline Run
                    </Button>
                  </TooltipTrigger>
                ...
                {isStagingDisabled && getDisabledButtonTooltip() && (
                  <TooltipContent side="top" align="center">
                    <p className="text-sm flex items-center gap-1"><Info className="h-4 w-4"/>{getDisabledButtonTooltip()}</p>
                  </TooltipContent>
                )}
              </Tooltip>
            </TooltipProvider>
