...
const commonRunInfoSchema = {
  run_name: z.string()
    ...
    .regex(/^[a-zA-Z0-9_ -]+$/, "Run Name can contain letters, numbers, spaces, underscores, and hyphens."),
...
const fastqPipelineSchemaBase = z.object({
  ...
  dbsnp: z.string().optional(), known_indels: z.string().optional(), pon: z.string().optional(),
  tools: z.array(z.string()).default([]), profile: z.enum(SAREK_PROFILES as [string, ...string[]]).default("docker"),
...
const pipelineInputSchema = z.discriminatedUnion("input_type", [ fastqPipelineSchemaBase, bamCramPipelineSchemaBase, vcfPipelineSchemaBase, ])
  .superRefine((data, ctx) => {
    ...
    if (isBqsrRelevant && isBqsrEnabled && missingBqsrFiles) { ctx.addIssue({ code: z.ZodIssueCode.custom, message: "dbSNP or Known Indels file required if BQSR not skipped.", path: ["dbsnp"] }); ctx.addIssue({ code: z.ZodIssueCode.custom, message: "dbSNP or Known Indels file required.", path: ["skip_baserecalibrator"] }); }
    if (data.step !== 'annotation') { const toolsToCheck = data.tools ?? []; const selectedSomaticTools = toolsToCheck.filter(tool => SOMATIC_TOOLS.includes(tool)); if (selectedSomaticTools.length > 0) { const hasTumorSample = data.samples.some(sample => sample.status === 1); if (!hasTumorSample) { ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Somatic tool(s) selected (${selectedSomaticTools.join(', ')}) require at least one sample with Status = 1 (Tumor).`, path: ["tools"] }); ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Tumor sample required for selected somatic tool(s).`, path: ["samples"] }); } } }
...
export default function InputPage() {
  ...
  const form = useForm<PipelineFormValues>({
    ...
    defaultValues: {
      ...
      genome: "GATK.GRCh38",
      step: "mapping",
      intervals_file: "", dbsnp: "", known_indels: "", pon: "",
  ...
  function onSubmit(values: PipelineFormValues) {
    const apiPayload: PipelineInput = {
      ...
      run_description: values.run_description,
      input_type: values.input_type,
      samples: values.samples.map((s): ApiSampleInfo => ({ patient: s.patient, sample: s.sample, sex: s.sex!, status: s.status!, lane: s.lane || null, fastq_1: s.fastq_1 || null, fastq_2: s.fastq_2 || null, bam_cram: s.bam_cram || null, index: s.index || null, vcf: s.vcf || null, })),
      ...
      wes: values.wes, trim_fastq: showTrimFastq ? values.trim_fastq : undefined,
  ...
  const scrollToFirstError = (errors: FieldErrors<PipelineFormValues>) => {
    ...
    if (errorKeys.length > 0) {
      ...
      if (firstErrorKey === 'samples') { const samplesCardHeader = formRef.current?.querySelector('#samples-card-header'); if (samplesCardHeader && errors.samples?.root) { samplesCardHeader.scrollIntoView({ behavior: "smooth", block: "center" }); return; } if (Array.isArray(errors.samples)) { const firstSampleErrorIndex = errors.samples.findIndex(s => s && Object.keys(s).length > 0); if (firstSampleErrorIndex !== -1) { const sampleErrors = errors.samples[firstSampleErrorIndex]; if (sampleErrors) { const firstSampleFieldError = Object.keys(sampleErrors)[0] as keyof ApiSampleInfo; fieldNameToQuery = `samples.${firstSampleErrorIndex}.${firstSampleFieldError}`; } } } else { fieldNameToQuery = 'samples.0.patient';} }
      ...
      if (isAdvancedField(fieldNameToQuery) && advancedAccordionValue !== "advanced-sarek-options") { setAdvancedAccordionValue("advanced-sarek-options"); requestAnimationFrame(attemptScroll); } else { attemptScroll(); }
    }
  };
  ...
  const handleProfileLoaded = (name: string | null, data: ProfileData | null) => {
    ...
    if (data) {
      ...
      if (loadedInputType !== currentFormInputType) { toast.info(`Profile '${name}' uses ${loadedInputType.toUpperCase()} input. Switching input type and applying settings.`); (formRef.current as any)._profileToApplyAfterReset = data; form.setValue('input_type', loadedInputType, { shouldValidate: true }); }
      ...
      form.reset(); setSelectedInputType('fastq'); setAdvancedAccordionValue(undefined);
    }
  };
  ...
  const handleSaveProfile = async (profileName: string) => {
    ...
    const profileData: ProfileData = {
      ...
      wes: currentValues.wes, trim_fastq: (showTrimFastq ? currentValues.trim_fastq : null),
