...
// --- Zod Schemas (Same as before - no changes needed here based on the plan) ---
...
// Final Schema with Refinements (Same as before)
...
export default function InputPage() {
  ...
  const form = useForm<PipelineFormValues>({
    ...
    defaultValues: {
      ...
      known_indels: "",
      pon: "",
      tools: [], // Default for tools
      profile: "docker", // Default for profile
      aligner: "bwa-mem", // Default for aligner
      joint_germline: false, // Default for joint_germline
      wes: false,
      trim_fastq: false, // Default for trim_fastq
      skip_qc: false, // Default for skip_qc
      skip_annotation: false, // Default for skip_annotation
  ...
  useEffect(() => {
      if (watchedInputType !== selectedInputType) {
          ...
          setAdvancedAccordionValue(undefined); // Collapse accordion on type change
      }
  ...
  const stageMutation = useMutation({
     ...
     onSuccess: (data) => {
        ...
        form.reset(); // Reset to original defaults
        setSelectedInputType('fastq'); // Reset selected input type state
        setCurrentProfileName(null);
        setAdvancedAccordionValue(undefined); // Collapse accordion
        router.push('/jobs');
     },
     onError: (error) => {
  ...
  const isAdvancedField = (fieldName: string): boolean => {
        // Simple check if field name starts with any advanced field name
        // (e.g., 'tools.0', 'profile')
        return ADVANCED_FIELD_NAMES.some(advField => fieldName.startsWith(advField));
  };
  ...
  const scrollToFirstError = (errors: FieldErrors<PipelineFormValues>) => {
        ...
        if (errorKeys.length > 0) {
            ...
            let fieldNameToQuery = firstErrorKey; // Default to the key itself

            if (firstErrorKey === 'samples') {
                 const samplesCardHeader = formRef.current?.querySelector('#samples-card-header');
                 if (samplesCardHeader && errors.samples?.root) { // Check for root error on samples array
                 ...
                 } else { fieldNameToQuery = 'samples.0.patient';} // Fallback
            } else { fieldNameToQuery = firstErrorKey; }


            const attemptScroll = () => {
                console.log(`Attempting to scroll to first error field: ${fieldNameToQuery}`);
                let element = formRef.current?.querySelector(`[name="${fieldNameToQuery}"]`);
                if (!element) {
                    const errorPathParts = fieldNameToQuery.split('.');
                    let selector = `#${errorPathParts.join('-')}-form-item`; // e.g., #step-form-item
                    ...
                    if (!element && fieldNameToQuery.startsWith('samples.')) {
                        // Fallback for sample fields: scroll to the sample card
                ...
                if (element) {
                    ...
                    console.warn(`Could not find element for error: ${fieldNameToQuery}. Scrolling form to top.`);
                    formRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
                }
            };

            // Check if the error is in an advanced field and expand accordion if needed
            if (isAdvancedField(fieldNameToQuery) && advancedAccordionValue !== "advanced-sarek-options") {
                ...
                requestAnimationFrame(attemptScroll); // Wait for render then scroll
            } else {
                attemptScroll(); // Scroll directly
            }
        }
    };
   ...
   const handleProfileLoaded = (name: string | null, data: ProfileData | null) => {
        ...
        if (data) {
            let loadedInputType: InputType = 'fastq'; // Default
            ...
            if (loadedInputType !== currentInputType) {
                 toast.info(`Profile '${name}' requires ${loadedInputType.toUpperCase()} input. Switching input type and applying settings.`);
                 form.setValue('input_type', loadedInputType, { shouldValidate: true }); // This will trigger the useEffect for input type change
                 // Values will be set by the useEffect after reset
            } else {
                // Same input type, just set values
                Object.entries(data).forEach(([key, value]) => {
                    const fieldKey = key as keyof ProfileData;
                    if (fieldKey in form.getValues()) {
                        // @ts-ignore
                        form.setValue(fieldKey, value !== null ? value : form.formState.defaultValues[fieldKey], {
                            shouldValidate: true,
                            shouldDirty: true
                        });
                         // Check if this loaded advanced field is different from its new default
                        if (ADVANCED_FIELD_NAMES.includes(fieldKey as keyof PipelineFormValues)) {
                            // @ts-ignore
                            const defaultValueForField = form.formState.defaultValues[fieldKey];
                            // Deep comparison for arrays (tools)
                            if (Array.isArray(value) && Array.isArray(defaultValueForField)) {
                                if (value.length !== defaultValueForField.length || !value.every(v => defaultValueForField.includes(v))) {
                                    shouldOpenAdvanced = true;
                                }
                            } else if (value !== defaultValueForField && value !== null) { // Only consider non-null loaded values as "changed"
                                shouldOpenAdvanced = true;
                            }
                        }
                    }
        ...
        <form ref={formRef} onSubmit={form.handleSubmit(onSubmit, onFormError)} className="space-y-8">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {fields.map((field, index) => {
               ...
               <FormMessage>{form.formState.errors.samples?.root?.message}</FormMessage> {/* For array-level errors */}
          ...
          <Card>
            <CardHeader>
                ...
                <CardDescription>Optional parameters to fine-tune the pipeline. Expand to see more.</CardDescription>
            </CardHeader>
            <CardContent>
                <Accordion type="single" collapsible className="w-full" value={advancedAccordionValue} onValueChange={setAdvancedAccordionValue}>
                    <AccordionItem value="advanced-sarek-options">
                        <AccordionTrigger className="text-base hover:no-underline">
                            <div className="flex items-center gap-2">
                                <Settings2 className="h-5 w-5" />
                                {advancedAccordionValue === "advanced-sarek-options" ? "Hide Advanced Options" : "Show Advanced Options"}
                            </div>
                        </AccordionTrigger>
                        <AccordionContent className="pt-6 space-y-6"> {/* Added pt-6 for spacing */}
                            {/* Tools (moved to advanced) */}
                            {showTools && (
                                <div>
                                    <div className="mb-4">
                            ...
                            <div className="space-y-4"> {/* Flags moved to advanced */}
            ...
            {/* Action Buttons */}
            <div className="flex justify-start items-center gap-4 pt-4"> {/* Added pt-4 for spacing */}
                 <TooltipProvider delayDuration={100}>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <span tabIndex={isStagingDisabled ? 0 : -1}> {/* For focusability when disabled */}
                                <Button
                                    type="submit"
