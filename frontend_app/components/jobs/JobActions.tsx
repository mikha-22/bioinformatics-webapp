            ...
            <div className="flex items-center justify-between gap-1 w-full">
                ...
                <div className="flex items-center gap-1">
                    <Button
                        ...
                        className={cn("h-9 w-9 p-2 flex items-center justify-center hover:bg-accent hover:text-accent-foreground transition-colors", !canViewLogs ? "opacity-50 cursor-not-allowed" : "cursor-pointer")}
                        title={canViewLogs ? "View Live Logs" : "Logs not available for staged jobs"} >
                        <Terminal className="h-5 w-5" /> <span className="sr-only">View Logs</span>
                    </Button>
                    {canStart && (
                        <Button variant="ghost" size="icon" onClick={handleStart} disabled={startMutation.isPending} className="h-9 w-9 p-2 flex items-center justify-center hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors" title="Start Job">
                            {startMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Play className="h-[24px] w-[24px]" />} <span className="sr-only">Start Job</span>
                        </Button>
                    )}
                    {canStop && (
                        <Button variant="ghost" size="icon" onClick={() => setIsStopConfirmOpen(true)} disabled={stopMutation.isPending} className="h-9 w-9 p-2 flex items-center justify-center hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors" title="Stop/Cancel Job">
                            {stopMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Square className="h-5 w-5" />} <span className="sr-only">Stop/Cancel Job</span>
                        </Button>
                    )}
                    {canRerun && (
                       <Button variant="ghost" size="icon" onClick={() => setIsRerunConfirmOpen(true)} disabled={rerunMutation.isPending} className="h-9 w-9 p-2 flex items-center justify-center hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors" title="Re-stage Job">
                           {rerunMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <RotateCcw className="h-5 w-5" />} <span className="sr-only">Re-stage Job</span>
                       </Button>
                    )}
                </div>
                <div className="flex items-center gap-1">
                ...
                <DialogContent className="sm:max-w-2xl">
                    ...
                    <div className="mt-4 max-h-[65vh] overflow-y-auto space-y-4 pr-2">
                         <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                            <div><span className="font-medium text-muted-foreground">Run Name:</span> <span className="font-semibold ml-1">{job.run_name || 'N/A'}</span></div> <div></div>
                            <div><span className="font-medium text-muted-foreground">Status:</span> <Badge variant={getStatusVariant(job.status)} className="capitalize ml-1">{job.status || 'Unknown'}</Badge></div> <div></div>
