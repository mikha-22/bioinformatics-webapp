...
// REMOVE Download from lucide-react if it's no longer used anywhere else in this file after the change
// Keep Loader2 if downloadRunMutation (for the whole folder) still uses it.
// For now, let's assume Download icon might still be used by the parent RunItem for the whole folder.
// If not, it can be removed from this specific import.
import { Folder, FileText, FileCode2, BarChart3, AlignLeft, ExternalLink, Loader2, Download } from "lucide-react";
...
// Sarek-specific file recognition (getFileIcon function remains the same)
const getFileIcon = (file: ResultItem) => {
    ...
    switch (ext) {
        case ".bam": case ".cram": return <AlignLeft className="h-4 w-4 text-blue-600 flex-shrink-0" />;
        case ".bai": case ".crai": return <AlignLeft className="h-4 w-4 text-blue-400 flex-shrink-0" />;
        case ".vcf": case ".bcf": case ".gz": case ".bgz": case ".csi": case ".tbi": return <FileCode2 className="h-4 w-4 text-purple-600 flex-shrink-0" />;
        case ".html": return <BarChart3 className="h-4 w-4 text-green-600 flex-shrink-0" />;
        case ".log": case ".txt": case ".out": case ".err": case ".report": case ".metrics": return <FileText className="h-4 w-4 text-gray-600 flex-shrink-0" />;
        case ".csv": case ".tsv": return <FileText className="h-4 w-4 text-lime-600 flex-shrink-0" />;
        case ".json": case ".yaml": case ".yml": return <FileCode2 className="h-4 w-4 text-pink-600 flex-shrink-0" />;
        case ".bed": return <FileCode2 className="h-4 w-4 text-rose-600 flex-shrink-0" />;
        case ".fa": case ".fasta": return <FileCode2 className="h-4 w-4 text-sky-600 flex-shrink-0" />;
        default: return <FileText className="h-4 w-4 text-gray-500 flex-shrink-0" />;
    }
};
...
interface FileActionsProps {
  file: ResultItem;
  runName: string; // Still needed if you keep "Open in File Browser"
}

function FileActions({ file, runName }: FileActionsProps) {
    // The downloadFileMutation is no longer needed here if we remove the button.
    // If you have other actions that might use mutations, keep it.
    // For now, let's comment it out or remove it if it's solely for the individual download.

    /*
    const downloadFileMutation = useMutation({
        mutationFn: () => api.downloadResultFile(runName, file.relative_path || file.name),
        onSuccess: (blob) => {
            try {
    ...
    */
    ...
    return (
        <div className="flex items-center gap-1">
            {/* --- REMOVED INDIVIDUAL FILE DOWNLOAD BUTTON --- */}
            {/*
            {!file.is_dir && (
                 <Button
                    variant="ghost"
                    ...
                    disabled={downloadFileMutation.isPending} // This would cause an error if mutation is removed
                >
            ...
            */}
             ...
             {/* View in FileBrowser Button (Keep this if desired) */}
...
export default function FileList({ files, runName }: FileListProps) {
  ...
  // Sarek-specific categorization logic (categorizedFiles memo remains the same)
  const categorizedFiles = React.useMemo(() => {
      ...
      files.forEach(file => {
          ...
          const name = file.name.toLowerCase();
          const ext = file.extension?.toLowerCase();
          const path = file.relative_path?.toLowerCase() || name;
  ...
  const renderFileItem = (file: ResultItem) => (
     <div key={file.relative_path || file.name} className="flex items-center justify-between p-2 hover:bg-accent/50 rounded-md text-sm group">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 text-xs text-muted-foreground ml-2">
             ...
             {/* FileActions component will now only show "Open in File Browser" if applicable */}
