...
from ..core.config import (
    ...
    RESULTS_DIR, LOG_HISTORY_PREFIX, DATA_DIR
)
...
async def stage_pipeline_job(
    ...
    try:
        ...
        job_details_for_redis = {
            ...
            "sarek_internal_description": input_data.description,
...
async def start_job(
    ...
    user_run_description = details.get("run_description") # This is the overall description for the run
    sarek_config_description = details.get("sarek_internal_description") # Sarek's specific config description
    ...
    meta_for_rq_job_obj = JobMeta(
        run_name=run_name,
        # Note: user_run_description is NOT part of JobMeta model directly.
        # It will be set on the RQ Job's 'description' attribute.
        ...
        sarek_params={
            ...
            "description": sarek_config_description, # Sarek's internal config description
        },
    ...
    try:
        rq_job = queue.enqueue(
            ...
            description=user_run_description # Set the RQ Job's own description field
        )
...
async def get_jobs_list(
        ...
        for job_id_bytes, job_details_bytes in staged_jobs_raw.items():
            ...
            try:
                ...
                all_jobs_for_frontend.append(JobStatusDetails(
                    ...
                    run_name=details.get("run_name"),
                    status="staged",
                    description=details.get("run_description"), # User's run_description
    ...
    for status_name, config in registries_config.items():
        try:
            ...
            if status_name in ["finished", "failed", "canceled"] and config["limit"] > 0 : ids_in_reg.reverse()
            ...
            for job in fetched_rq_jobs:
                if job:
                    ...
                    if current_status == JobStatus.FAILED:
                        ...
                        if job.exc_info and (meta_obj.error_message == "Job failed" or not meta_obj.error_message) :
                    ...
                    all_jobs_for_frontend.append(JobStatusDetails(
                        ...
                        description=job.description, # Use RQ Job's own description for run_description
...
async def stop_job(
    job_id: str,
    redis_conn: redis.Redis = Depends(get_redis_connection), # For general Redis ops if needed
    queue: Queue = Depends(get_pipeline_queue) # For RQ specific ops
):
    if job_id.startswith("staged_"):
    ...
    try:
        job = Job.fetch(job_id, connection=queue.connection) # Use queue's connection
        status = job.get_status(refresh=True)
        if job.is_finished or job.is_failed or job.is_stopped or job.is_canceled:
            ...
            # This connection should be derived from the queue's connection parameters
            redis_conn_for_command = redis.Redis(
                ...
                decode_responses=False # send_stop_job_command might expect bytes
            )
...
async def remove_job(
    ...
    if job_id.startswith("staged_"):
        ...
        elif not details_bytes: # If hdel returned 0 and it wasn't there to begin with
            logger.warning(f"Staged job '{job_id}' not found for removal (already gone).")
            job_removed_flag = True # Consider it removed if not found
        else: # hdel returned 0 but it was there - should not happen
             logger.error(f"Failed to remove staged job {job_id} despite it existing.")
    else: # RQ Job
        try:
            ...
            logger.warning(f"RQ Job '{job_id}' not found for removal (might be already removed).")
            job_removed_flag = True # If not found, consider it removed for cleanup purposes
        except Exception as e:
    ...
    if job_removed_flag:
        ...
        if not job_id.startswith("staged_"): # Only for RQ jobs that might have logs
        ...
        logger.warning(f"Job {job_id} not found or removal failed.")
        raise HTTPException(status_code=404, detail=f"Job {job_id} not found for removal or removal failed.")
...
async def rerun_job(
    ...
    try:
        ...
        # Use original_job.description for the user's run description
        ...
        new_rerun_user_description = f"Re-run of '{original_run_name}'. Original desc: {original_user_run_description or 'N/A'}"
        ...
        for i, sample_data_dict in enumerate(original_meta.sample_info):
            ...
            def _resolve(path_str: Optional[str], is_req: bool, allowed_exts: Optional[List[str]] = None) -> Optional[str]:
                if not path_str:
                    if is_req: validation_errors_rerun.append(f"Sample {i+1}: Missing required path string."); return None
                    return "" # Return empty string for optional non-provided paths in CSV
                try:
                    abs_p = get_safe_path(DATA_DIR, path_str)
                    if not abs_p.is_file(): validation_errors_rerun.append(f"Sample {i+1}: File '{path_str}' (abs: {abs_p}) not found."); return None
                    if allowed_exts and not any(abs_p.name.lower().endswith(ext.lower()) for ext in allowed_exts):
                        validation_errors_rerun.append(f"Sample {i+1}: File '{path_str}' has invalid extension. Allowed: {', '.join(allowed_exts)}"); return None
                    return str(abs_p)
                except HTTPException as e_http: validation_errors_rerun.append(f"Sample {i+1}: Path error for '{path_str}': {e_http.detail}"); return None
                except Exception as e_gen: validation_errors_rerun.append(f"Sample {i+1}: General error for '{path_str}': {str(e_gen)}"); return None
            ...
            if original_meta.input_type == "fastq":
                ...
                current_row_values.extend([sample_obj.lane, _resolve(sample_obj.fastq_1, True, ['.fq.gz','.fastq.gz','.fq','.fastq']), _resolve(sample_obj.fastq_2, True, ['.fq.gz','.fastq.gz','.fq','.fastq'])])
            elif original_meta.input_type == "bam_cram":
                bc_p = _resolve(sample_obj.bam_cram, True, ['.bam','.cram'])
                idx_p = _resolve(sample_obj.index, bool(bc_p and bc_p.endswith('.cram')), ['.bai','.crai'])
                current_row_values.extend([bc_p, idx_p])
            elif original_meta.input_type == "vcf":
                vcf_p_val = _resolve(sample_obj.vcf, True, ['.vcf','.vcf.gz'])
                idx_p_val = _resolve(sample_obj.index, bool(vcf_p_val and vcf_p_val.endswith('.vcf.gz')), ['.tbi','.csi'])
        ...
        sarek_p_orig = original_meta.sarek_params or {}
        ...
        new_job_details_for_redis = {
            ...
            "intervals_path": _resolve(input_p_orig.get("intervals_file"), False) if input_p_orig.get("intervals_file") else None, # Resolve to absolute for task
            "dbsnp_path": _resolve(input_p_orig.get("dbsnp"), False) if input_p_orig.get("dbsnp") else None,
            "known_indels_path": _resolve(input_p_orig.get("known_indels"), False) if input_p_orig.get("known_indels") else None,
            "pon_path": _resolve(input_p_orig.get("pon"), False) if input_p_orig.get("pon") else None,
