...
# APP_NOTIFICATIONS_CHANNEL is used for job status updates
...
def run_pipeline_task(
    ...
    try:
        ...
        try:
            ...
            # <<< --- ADDED: Publish "job_started" notification --- >>>
            if job and redis_log_handler:
                start_notification_payload = {
                    "event_type": "job_started",
                    "job_id": job_id,
                    "run_name": job.meta.get('run_name', current_run_name), # Use run_name from meta if available
                    "message": f"Job '{job.meta.get('run_name', current_run_name)}' has started processing.",
                    "status_variant": "info" # Or "warning" if you prefer yellow for started
                }
                try:
                    redis_log_handler.publish(APP_NOTIFICATIONS_CHANNEL, json.dumps(start_notification_payload))
                    logger.info(f"[Job {job_id}] Published 'job_started' notification to '{APP_NOTIFICATIONS_CHANNEL}'.")
        ...
        while streams_to_select:
            ...
            if return_code is not None and not streams_to_select:
                 ...
                 break
            try:
                 readable, _, _ = select.select(streams_to_select, [], [], 0.2)
                    ...
                    if stream_item is process.stdout:
                        ...
                        while '\n' in stdout_buffer:
                            ...
                            if match_nf_progress and job:
                                ...
                                job.meta['submitted_task_count'] = nf_total
        ...
        if job and trace_file_path_obj and trace_file_path_obj.is_file():
            ...
            elif job.meta.get('overall_progress') is None and return_code == 0 :
        ...
        try:
            if job_id and not job_id.startswith("N/A") and redis_log_handler:
                ...
                if job: 
                    ...
                    # Construct user message for notification
